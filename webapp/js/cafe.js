...
var Cafe = {
    ...
    init: function (options) {
        ...
        var userId = Telegram.WebApp.initDataUnsafe && Telegram.WebApp.initDataUnsafe.user && Telegram.WebApp.initDataUnsafe.user.id || Cafe.userId;
        if (options.debug) {
    ...
    toggleMode: function (mode_order) {
        ...
        if (mode_order) {
            ...
            $('.js-order-address-field').each(function () {
                autosize.update(this);
            });
    ...
    mainBtnClicked: function () {
        ...
        if (Cafe.modeOrder) {
            ...
            let form = document.getElementById("js-order-form");
    ...
    apiRequest: function (method, data, onCallback) {
        var authData = Telegram.WebApp.initDataUnsafe || '';
        // $.ajax(Cafe.apiUrl, {
        //     type: 'POST',
        ...
        fetch(`${Cafe.apiUrl}/${method}`, {
            method: "POST",
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify($.extend(data, {_auth: authData}))
        })
            .then(res => res.json())
            .then(data => {
